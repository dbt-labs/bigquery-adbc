...
func (c *connectionImpl) newClient(ctx context.Context) error {
	...
	case OptionValueAuthTypeExternalAccount:
		if c.externalAccountAudience == "" {
			return adbc.Error{Code: adbc.StatusInvalidArgument, Msg: fmt.Sprintf("[bq] `%s` parameter is empty", OptionStringAuthExternalAccountAudience)}
		}
		if c.externalAccountRequestURL == "" {
			return adbc.Error{Code: adbc.StatusInvalidArgument, Msg: fmt.Sprintf("[bq] `%s` parameter is empty", OptionStringAuthExternalAccountRequestURL)}
		}
		if c.externalAccountRequestData == "" {
			return adbc.Error{Code: adbc.StatusInvalidArgument, Msg: fmt.Sprintf("[bq] `%s` parameter is empty", OptionStringAuthExternalAccountRequestData)}
		}
